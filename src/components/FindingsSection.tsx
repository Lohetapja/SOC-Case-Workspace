 ...
 */
export function FindingsSection({ findings, evidence, timeline, onAdd, onRemove }: FindingsSectionProps) {
          ...
          {findings.map((finding) => {
            ...
            return (
              <li key={finding.id} className="detail-item">
                ...
                {supporting.length > 0 && (
                  <p className="detail-item__meta">Supported by: {supporting.join(', ')}</p>
                )}
                {relatedEvents.length > 0 && (
                  <p className="detail-item__meta">Timeline: {relatedEvents.join(', ')}</p>
                )}
              </li>
            )
          })}
